    ...
    <div class="search_result">
      ...
      <ul>
        <li
          ...
          :key="film.id"
        >
          <div class="img"><img :src="film.img|image('128.180')"></div>
          <div class="info">
            <p><span>{{film.nm}}</span><span>{{film.sc}}</span></p>
            <p>{{film.enm}}</p>
            <p>{{film.cat}}</p>
            <p>{{film.rt}}</p>
          </div>
...
export default {
  ...
  watch: {
    searchname(newvalue) {
		var that=this;
      	this.cancelRequest();
      	this.axios.get("/ajax/search?kw=" + newvalue + "&cityId=212&stype=-1", {
          	cancelToken: new this.axios.CancelToken(function(c) {
			//所有的axios都加了this.axios  是因为我们是在main.js中把axios配置到了vue的属性上，所以才加的
			//如果是在页面中import导入的axios，那么不用加this
            that.source = c;
          }),
        }).then((res) => {
          this.filmslist = res.data.movies.list;
        }).catch((err) => {
